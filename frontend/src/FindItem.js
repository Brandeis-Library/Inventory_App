...
class FindItem extends React.Component {
  ...
  async callAPI() {
    let { data } = await axios.post('http://localhost:9000/retreiveItem', {
    ...
    if (data.name !== 'Error') {
      ...
      let data2 = await axios.put(
        'http://localhost:9000/updateItemInventoryDate',
        {
          ...this.state.dataObj,
          mmsId: this.state.mms_id,
      ...
      let data3 = await axios.put('http://localhost:9000/holdingsData', {
        mmsId: this.state.mms_id,
        holdingId: this.state.holdingID,
      });
    ...
    return (
      <div>
        ...
        <p>ItemID: {this.state.itemID}</p> */}
          <p>
            Library/Location: {this.state.permLib} - {this.state.permLoc}
          </p>

          {this.state.tempLib ? (
            <p>
              <strong>
                Temp Library/Location: {this.state.tempLib} -{' '}
                {this.state.tempLoc}
              </strong>
            </p>
          ) : (
          ...
          {this.state.string583a ? (
            <p>
              <strong>583a field: {this.state.string583a}</strong>
            </p>
          ) : (
            <div></div>
          )}
